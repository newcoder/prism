...
function ucci(path, options) {
  this.path = path || 'eleeye.exe';
  this.options = _extend(ucciOptions, options) ;
  this.defaults = {};
...
ucci.prototype.applyOptions = function() {
  for (opt in this.options) {
    if (this.defaults[opt] && (this.defaults[opt].value !== this.options[opt]) )
...
ucci.prototype.loadEngine = function (path) {	
  ...
  var self = this;
  this.child = exec(this.path, function (error, stdout, stderr) {
    if (error !== null) {
	  self.emit('engineError', error);
	}	
  });
...
ucci.prototype.isCommandAccept = function(cmdName) {
  return CommandStateMap[this.state].indexOf('' + cmdName + '') >= 0;
}
...
ucci.prototype.setoption = function(name, value) {
  this.sendCommand('setoption ' + name + ' ' + value);
}
...
ucci.prototype.go = function(option, modes) {
  ...
  var sent = this.sendCommand('go ' + opt + ' ' + modes);
  if (sent) {
...
ucci.prototype.unloadEngine = function() {
  if (this.child && (this.status == EngineStatus.quit))
    this.child.kill();
}
