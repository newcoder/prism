...
TEST_F(TestTimeSeries, testThreshold)
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		TLUtils::Dump(kDataPath + std::to_string(static_cast<long long>(i))+ "_dump_OriginalAndExtrema.csv", &tl, &result);
		double dist = TLUtils::Distance(&tl, &result);
